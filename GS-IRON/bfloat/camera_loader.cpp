...
void load_camera(Camera& cam, Eigen::Matrix4f baseMat_W2C){
    ...
    cam.full_proj = float_to_bfloat_mat4f(proj_mat * cam.world_to_view);
    std::cout << "test" << 1.2f << " " << float_to_bfloat16_scale(1.2f) << "\n";

    cam.world_to_view = float_to_bfloat_mat4f(cam.world_to_view);
    
    
    cam.pos = float_to_bfloat_vec3f(cam.world_to_view.transpose().inverse().block<1,3>(3,0));
}