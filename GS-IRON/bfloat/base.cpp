...
void render(std::string ply_name, Eigen::Matrix4f baseMat_W2C, std::string img_name){
    ...
    for(int i=0;i<numGaussians;i++){
        ...
        R = float_to_bfloat_mat3f(R);
        
        Eigen::Matrix3f S;
        S << std::exp(g.scale[0]), 0.0f, 0.0f,
              0.0f, std::exp(g.scale[1]), 0.0f,
              0.0f, 0.0f, std::exp(g.scale[2]);
        S = float_to_bfloat_mat3f(S);

        Eigen::Matrix3f M;
        M = float_to_bfloat_mat3f(R * S);
        Eigen::Matrix3f covariance3D =  float_to_bfloat_mat3f(M * M.transpose());
        ...
        // calc eignvals
    	float b = float_to_bfloat16_scale(0.5f * (covariance2D(0,0) + covariance2D(1,1)));
        // short/long diameter
	    float lambda1 = float_to_bfloat16_scale(b + std::sqrt(std::max(0.1f, b * b - det)));
	    float lambda2 = float_to_bfloat16_scale(b - std::sqrt(std::max(0.1f, b * b - det)));
	    float radius = float_to_bfloat16_scale(std::ceil(3.f * std::sqrt(std::max(lambda1, lambda2))));
        // get related tiles
    	...
    	float y = dif[1];
	    float z = dif[2];
		float xx = float_to_bfloat16_scale(x * x), yy = float_to_bfloat16_scale(y * y), zz = float_to_bfloat16_scale(z * z);
		float xy = float_to_bfloat16_scale(x * y), yz = float_to_bfloat16_scale(y * z), xz = float_to_bfloat16_scale(x * z);
        for(int i=0;i<=2;i++){
            ...
            //clamp
            colors[i] = float_to_bfloat16_scale(std::max(0.0f, colors[i]));
        }
        g.color = colors;
