    ...
    for(int i=0;i<count;i++){
        ...
        for(size_t j=0;j<propertyNames.size() ;j++){
            ...
            //note that these scalings are log-based
                ...
                gaussian.rotation[0] = float_to_bfloat16_scale(value);
            }else if(propName == "rot_1"){
                gaussian.rotation[1] = float_to_bfloat16_scale(value);
            }else if(propName == "rot_2"){
                gaussian.rotation[2] = float_to_bfloat16_scale(value);
            }else if(propName == "rot_3"){
                gaussian.rotation[3] = float_to_bfloat16_scale(value);
            }else if(propName == "nx"){
                ...
                gaussian.opacity = float_to_bfloat16_scale(1 / (1 + std::exp(-value))); //sigmoid
            }else if(propName == "f_dc_0"){
                gaussian.f_dc[0] = float_to_bfloat16_scale(value);
            }else if(propName == "f_dc_1"){
                gaussian.f_dc[1] = float_to_bfloat16_scale(value);
            }else if(propName == "f_dc_2"){
                gaussian.f_dc[2] = float_to_bfloat16_scale(value);
            }else if(propName.rfind("f_rest_", 0) == 0){
                int idx = std::stoi(propName.substr(7));
                gaussian.f_rest[idx] = float_to_bfloat16_scale(value);
            }

        }
        gaussian.xyz = float_to_bfloat_vec3f(gaussian.xyz);
        gaussian.scale = float_to_bfloat_vec3f(gaussian.scale);
        gaussian.normalxyz = float_to_bfloat_vec3f(gaussian.normalxyz);
